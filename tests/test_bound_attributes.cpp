...
static ElementPtr parse(const QString& xml)
{
    return XmlLayoutParser::parseString(
        QStringLiteral("<root xmlns:b=\"urn:broaditem:binding\">") + xml +
        QStringLiteral("</root>"));
}
...
static std::unique_ptr<Node> materializeFirst(const ElementPtr& root, MapPropertyContext& map)
{
    LayoutContext ctx;
    ctx.ctx = &map;
