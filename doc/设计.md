...
控制部件（`<for>`、`<if-has>`）在解析阶段以模板层树节点形式存在，但在**物化阶段**被**透明地展开**为实例层节点序列，直接参与父容器的统一布局。这一过程称为**运行时展平**。
...
- `<if-has>` 根据条件判断结果，物化出子元素的节点序列或返回空序列。
- 普通元素物化为单个节点。
...
if-has部件用于条件渲染；在物化时，若条件满足则物化出子元素的节点序列，否则返回空序列，由父容器统一处理。
