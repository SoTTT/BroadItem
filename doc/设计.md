...
3. **显式 `width`/`height`**：覆盖内容固有尺寸成为请求尺寸，内容超出请求尺寸的部分被裁剪；
...
- layout：row 未显式指定宽度，被 column 交叉轴拉伸至列宽，内部出现剩余空间，`main-align="end"` 将两个 text 整体推到右端；底部的长文本宽度等于列宽，无剩余，按 start 放置；
...
- **单一口径**：全部诊断经 `ErrorCollector` 接口投递；进程级唯一收集器可注入，默认实现格式化后转发 `qWarning`（行为与现状兼容）；解析期可经 `parse` 参数叠加本次调用专用收集器。库内除默认收集器外不出现裸 `qWarning`/`qCritical`。
- **解析期**：同一错误实例恰好报告一次。
...
| BI-P-017 | Warning | Default | grid 子元素超容量（保留全部） |
