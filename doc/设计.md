...
3. **显式 `width`/`height`**：覆盖内容固有尺寸成为请求尺寸，内容超出请求尺寸的部分被裁剪（省略即未指定；"未指定"在实现层以 -1 哨兵表示，不属用户可书写语义；`<rect>` 无"覆盖"一说，显式声明即其固有尺寸）；
...
- **编译特性**：`cxx_std_17` 作为 PUBLIC compile feature 随导出目标传导（全局 `CMAKE_CXX_STANDARD` 不进导出目标，故显式声明）。
