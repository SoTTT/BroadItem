...
在视图中，允许绑定属性——即 xml 中的元素可以通过 `bind`、`for`、`if-has` 等属性引用外部数据。动态属性值的类型限制为 `QString` 和 `QStringList`，前者用于大多数属性绑定（如 `<text bind="title">`），后者用于 `<for bind="list">` 的列表遍历。
...
**空值语义**：`setProperty(name, QVariant())` 或 `QVariant(QVariant::Invalid)` 表示移除属性——该键从 `QVariantMap` 中删除，`hasProperty(name)` 返回 `false`。这使 `<if-has bind="warning">` 的条件隐藏自然运作：设置空值即可隐藏内容。
...
- `bind="device.cpu"` 的元素会被 `bindsProperty("device")` 命中（前缀匹配 `device.`）
- `bind="processes[0].name"` 的元素会被 `bindsProperty("processes")` 命中（前缀匹配 `processes[`）
- 扁平键仍按精确匹配
...
|    bind     | 绑定一个属性，将其值作为content | 属性值必须为 `QString`，类型不匹配打印 `qCritical` 并回退到静态内容 |
...
| 属性 |                    含义                    |                  备注                   |
| :--: | :----------------------------------------: | :-------------------------------------: |
| bind | 绑定一个列表，这个列表的值将作为循环的元素 | 属性值必须为 `QStringList`，类型不匹配打印 `qCritical` 并返回空列表；未设置时也返回空列表 |
| step |               遍历使用的步长               |                                         |
