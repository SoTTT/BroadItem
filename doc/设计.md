...
1. **内容固有尺寸**：`<text>` 由文字度量决定（字体、字号、`wrap`/`max-width`），`<image>` 由图像原始尺寸决定（单维指定时按宽高比推导另一维）；
2. **盒模型装饰**：容器尺寸 = content + padding + border-width×2 + margin；
3. **显式 `width`/`height`**：覆盖内容固有尺寸成为请求尺寸，内容超出请求尺寸的部分被裁剪（省略即未指定；"未指定"在实现层以 -1 哨兵表示，不属用户可书写语义）；
...
3. **交叉轴定位**：`cross-align` 为 start/center/end 时按容器交叉轴边界定位子矩形；为 stretch 时子矩形在交叉轴上与容器内容区重合（尺寸已被拉满，位置随之确定）；为 baseline（仅 row）时不做拉伸，各子元素自报顶边到基线的距离（`<text>` 为 margin-top + border-width + padding-top + 首行 ascent，其余元素无基线、回退为以底边参与），取最大基线作为对齐线，子矩形 y = 内容区顶 + (最大基线 − 自身基线)，行内容高 = 最大基线 + 最大（子高 − 基线）；
