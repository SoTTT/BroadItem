...
保留属性 `of`/`prop`/`as`/`content` 走控制元素与文本内容的专用通道（`<for>`/`<if-has>` 伪属性包装、`TextElement` 内容绑定），不进入通用绑定表。
...
- `b:content` 例外：路径解析失败时渲染空字符串并输出 `qWarning()`；类型不匹配时输出 `qCritical`，回退到标签内默认文本。
