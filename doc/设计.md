...
- **解析期**：同一错误实例恰好报告一次。
- **运行时**：同一（模板 × 错误码 × 绑定路径）组合只报告一次（`ImageElement::m_failedPaths` 模式推广为模板级 reported-set），防止秒级刷新下同一条错误刷屏。
...
实现状态（P1-1 已落地）：诊断设施位于 `diagnostics/` 模块——`Diagnostic.h`（错误码枚举 + 码表）、`ErrorCollector.h`（接口 + 默认实现）、`Diagnostics.h`（`reportParse`/`reportRuntime` 入口与 `ParseSession`/`RuntimeScope` 作用域）。`XmlLayoutParser::parseFile/parseString` 接受可选的本次调用收集器；运行时去重由 `Frame::performLayout/paint` 安装的 `RuntimeScope` 承担，集合宿主为模板根元素（`Element::markDiagnosticReported`）。历史个例（"未知标签报 warning 却 abort""互斥属性报 critical 却继续"）已按码表正名。配套命令行校验器暂缓，后续单独立项。
