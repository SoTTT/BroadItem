...
对于**场景位置观察模式**，`ReactiveBinding` 不再连接普通 Q_PROPERTY NOTIFY 信号，而是创建内部 `ScenePosTracker`。`ScenePosTracker` 从目标 `QGraphicsObject` 出发，沿 `parentItem()` 向上遍历所有 `QGraphicsObject` 祖先，分别连接 `xChanged()` / `yChanged()` 以及 `scaleChanged()` / `rotationChanged()` / `visibleChanged()` / `opacityChanged()`；仅对目标自身连接 `parentChanged()`，以便目标重新父级化时重建整条父链监听。构造时还会自动为目标及其祖先启用 `ItemSendsGeometryChanges | ItemSendsScenePositionChanges` 标志。
...
| 方法 | 说明 |
|------|------|
| `static ReactiveBinding* create(QObject* source, srcProp, target, tgtProp, transform)` | 静态工厂：创建绑定。校验失败返回 `nullptr` 并输出 `qWarning` |
...
- 仅监听目标自身的 `parentChanged()`；目标重新父级化时，`ScenePosTracker` 会断开旧连接并重新遍历新的父链。
...
| `scenePos`（场景坐标） | 无独立 NOTIFY | `ScenePosTracker` 递归监听目标及其 `QGraphicsObject` 祖先的 `xChanged()`、`yChanged()`、`scaleChanged()`、`rotationChanged()`、`visibleChanged()`、`opacityChanged()`；对目标监听 `parentChanged()`；对祖先监听 `destroyed()` |
...
除基础的 `create()` 属性绑定外，响应式模块还提供三个直接基于 `scenePos()` 语义工作的可视化组件。它们都依赖 `createScenePosObserver()` 来捕获嵌套 parent 链中的真实世界坐标变化。
...
- 通过 `ReactiveBinding::createObserver()` 监听 decorated 的 `pos` 变化以同步几何体；在 decorated 具备带 NOTIFY 信号的 `width` / `height` 时额外连接这两个属性。
