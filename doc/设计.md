...
不变量：**`parse()` 返回 `nullptr` ⇔ 发生了至少一个 Abort 级错误**。解析期为全收集语义——遇到 Abort 不提前返回，走完整个 DOM 把所有错误收集完毕后统一交付；全收集只影响诊断完整性，不改变上述返回契约。实现注意：Abort 发生后不再构建结果树，现行实现中"子元素解析失败由父容器静默跳过、可能返回裁剪树"的行为在 P1-1 落地时一并修正。
...
| 运行时错误 | 绑定路径 + 触发属性名（无 file/line 概念） |
...
英文、模板化；错误码定义消息模板，填充值统一以引号包裹；不带模块名前缀（模块由错误码段表达）。
