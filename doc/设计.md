...
- `<for>` 根据绑定的列表数据，为每个迭代项创建项级 `MapPropertyContext`（通过 `:as` 变量名绑定当前值），链接到全局上下文后对模板执行 `clone()` + `resolveBindings()`，生成多个独立实例。
...
|----------------|--------|----------------------|-----------|
| `:content` | `TextElement` | `QVariant::String` | `qCritical`，回退到标签内默认文本 |
| `:of` | `ForElement` | `QVariant::StringList` 或 `QVariant::List` | `qCritical`，返回空列表（无循环输出） |
| `:as` | `ForElement` | `QVariant::String`（变量名） | 强制属性，缺失时 `qCritical` 并返回空列表 |
| `:prop` | `IfHasElement` | 无要求 | 仅检查 `hasProperty()`，不读取值 |
...
1. **项级上下文** — 当前迭代项的数据（`:as` 变量名对应的值）。支持嵌套路径：若迭代值为 `QVariantMap`，其字段以平铺键写入项级上下文，可直接通过 `字段名` 或 `变量名.字段名` 访问
2. **全局上下文** — 若项级上下文中未找到，回退到父容器的全局 `PropertyContext`

例如，`<for :of="users" :as="user">` 中绑定 `:content="user.name"`：
...
若路径解析失败（类型不匹配、键不存在、路径语法错误等），输出 `qWarning` 并返回空字符串。`:as` 变量名本身不是全局属性（`bindsProperty` 会剥离 `:as` 前缀），修改全局同名字段不会触发 per-item 重绘。
...
| :of  | 引用 PropertyContext 中的列表属性            | 属性值必须为 `QStringList` 或 `QVariantList`，类型不匹配打印 `qCritical` 并返回空列表；未设置时也返回空列表 |
| :as  | 迭代变量名（**强制**）                       | 指定每次迭代将当前项值绑定到的变量名，模板中子元素通过 `:属性名="变量名.字段"` 引用当前项数据 |

`:as` 是强制属性。缺失时 `ForElement::expand()` 输出 `qCritical` 并返回空列表。

##### 内容

for 标签的内容是**单个模板部件**。通过 `:as` 指定的变量名，模板中的绑定属性可引用当前迭代项的值：

```xml
<column space="8">
  <for :of="users" :as="user">
    <row space="4">
...
```xml
<for :of="users" :as="u">
  <text :content="name"/>   <!-- 等价于 u.name，无需前缀 -->
</for>
```
...
若路径解析失败（类型不匹配、键不存在等），输出 `qWarning` 并返回空字符串。`:as` 变量名本身不是全局属性（`bindsProperty` 会剥离 `:as` 前缀），修改全局同名字段不会触发 per-item 重绘。
...
> **注意**：伪属性 `:as` 仅当标签同时具有 `:of` 伪属性且标签非 `<for>` 时生效。若标签有 `:as` 但无 `:of`，解析器输出 `qWarning`。`<for>` 元素自身的 `:as` 始终作为直接属性解析。
