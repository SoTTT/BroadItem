...
`ObservableGraphicsObject` 是 `QGraphicsObject` 的子类。在纯元对象驱动架构下，它**不再声明任何自定义信号**。其唯一职责是在构造函数中设置 `ItemSendsGeometryChanges | ItemSendsScenePositionChanges` 标志，确保 Qt 内部的 `xChanged()`/`yChanged()` 等 NOTIFY 信号能够正常发射。

其他属性（scale、rotation、opacity、visible）的信号由 `QGraphicsObject`/`QGraphicsItem` 内部的 `Q_PROPERTY` NOTIFY 机制自行发出，不需要本类额外处理。
...
**类型泛化**：`create()` 的参数类型为 `QObject*`（而非 `ObservableGraphicsObject*`），允许任何 `QObject` 子类作为源或目标，只要其目标属性满足元对象校验条件。
