...
- `<for>` 根据绑定的列表数据，为每个迭代项创建项级 `MapPropertyContext`（通过 `b:as` 变量名绑定当前值），链接到全局上下文后对模板执行 `clone()` + `resolveBindings()`，生成多个独立实例。
...
> 包括 `<for>` 和 `<if-has>` 自身。如果不加防护，`<for b:of="list">` 会被**双重包装**：
>
...
所有 XML 属性默认均为**字面量语义**。通过命名空间前缀绑定属性（如 `b:content="title"`）将属性声明为**引用语义**，值将到 `PropertyContext` 中查找。
...
|----------------|--------|----------------------|-----------|
| `b:content` | `TextElement` | `QVariant::String` | `qCritical`，回退到标签内默认文本 |
...
| `b:prop` | `IfHasElement` | 无要求 | 仅检查 `hasProperty()`，不读取值 |
...
- 同一属性不能同时存在字面量和引用形式（如 `content="hello"` 和 `b:content="title"` 同时出现），解析时抛出错误。
...
**空值语义**：`setProperty(name, QVariant())` 表示移除属性——该键从 `QVariantMap` 中删除，`hasProperty(name)` 返回 `false`。这使 `<if-has prop="warning">` 的条件隐藏自然运作：设置空值即可隐藏内容。
...
> **注意**：Q_PROPERTY 是编译期声明的，始终存在（`hasProperty` 对已声明属性恒为 `true`）。`hasProperty` 对运行时动态属性同样返回 `true`。`null` 值（如 `QString()` 的默认值）在 `<if-has>` 中被视为不存在——`QString()` 默认值的属性不会触发条件显示。若需要区分"显式设 null"和"未初始化"，使用 `MapPropertyContext`。
...
`b:as` 是强制属性。缺失时 `ForElement::expand()` 输出 `qCritical` 并返回空列表。
...
每次迭代，`ForElement::expand()` 为当前项创建一个独立的 `MapPropertyContext`，并通过 `ItemPropertyContext` 链接到全局 `PropertyContext`。模板元素的绑定属性解析优先级为：
...
| b:prop | 引用 PropertyContext 中的属性 | 若该属性存在则渲染，否则隐藏 |
...
响应式绑定系统提供了一种 **Item 级别**（`QGraphicsItem` 层）的属性单向同步机制。与 Element 级别的数据绑定（通过 `PropertyContext` 和 XML 中的 `b:content` 等绑定属性）不同，响应式绑定作用于 `QGraphicsItem` 的 Qt 原生属性（位置、缩放、旋转、透明度、可见性），允许两个 `BroadItem`（或任何 `ObservableGraphicsObject` 子类）之间建立实时的、由信号驱动的属性跟随关系。
...
| 数据源 | PropertyContext（QVariantMap / Q_PROPERTY） | ObservableGraphicsObject 信号 |
...
> **边界声明**：响应式绑定系统**未**与 `PropertyContext` 或 Element 级别的绑定机制集成。在 XML 布局文件中**没有**声明响应式绑定的语法——绑定关系必须通过 C++ 代码建立。这只是第一版实现，未来可能扩展。
...
```
源 ObservableGraphicsObject         ReactiveBinding                目标 ObservableGraphicsObject
────────────────────────────         ──────────────                ────────────────────────────
QGraphicsItem 属性变化               ① 信号连接                      目标 setter 被调用
    ↓                                    ↓                              ↓
itemChange() 检测差异              ② 读取源属性值                  setPos() / setScale()
    ↓                                    ↓                         setRotation() / setOpacity()
emit positionChanged()             ③ 应用变换函数（可选）            setVisible()
emit scaleChanged()                     ↓
emit rotationChanged()             ④ 类型检查
emit opacityChanged()                   ↓
emit visibilityChanged()           ⑤ 写入目标属性
                                        ↓
                                    循环检测（重入保护）
```

核心流程：
1. 当源 `ObservableGraphicsObject` 的 `QGraphicsItem` 属性变化时，`itemChange()` 检测实际数值差异并发射对应信号。
2. `ReactiveBinding` 在构造时通过 `connectSourceSignal()` 订阅源的对应信号。
3. 信号触发 → `evaluate()` 执行：读取源值 → 应用变换函数 → 写入目标。
4. 写入目标属性会调用 `setPos()` 等 setter，再次触发目标对象的 `itemChange()`，但此时 `ReactiveBinding` 的 `m_evaluating` 标志已置位，不会形成级联。

### ObservableGraphicsObject 类

`ObservableGraphicsObject` 是 `QGraphicsObject` 的子类，为 BroadItem 提供属性变化可观察能力。它重写了 `itemChange()` 虚函数，检测五种属性的实际数值变化并发射对应信号。

**构造函数**：
- `ObservableGraphicsObject(QGraphicsItem* parent = nullptr)`：构造时自动设置 `ItemSendsGeometryChanges` 和 `ItemSendsScenePositionChanges` 标志，确保 `itemChange()` 能被调用。

**信号（5 个）**：

| 信号 | 参数 | 触发条件 |
|------|------|----------|
| `positionChanged(const QPointF& newPos)` | 新位置 | `ItemPositionHasChanged` |
| `scaleChanged(qreal newScale)` | 新缩放比例 | `ItemScaleHasChanged` |
| `rotationChanged(qreal newRotation)` | 新旋转角度 | `ItemRotationHasChanged` |
| `opacityChanged(qreal newOpacity)` | 新不透明度 | `ItemOpacityHasChanged` |
| `visibilityChanged(bool visible)` | 新可见性 | `ItemVisibleHasChanged` |

`BroadItem` 继承自 `ObservableGraphicsObject`（`class BroadItem : public ObservableGraphicsObject`），因此每个 BroadItem 实例都天然具备被观察和作为绑定源/目标的能力。
...
| 方法 | 说明 |
|------|------|
| `static ReactiveBinding* create(source, srcProp, target, tgtProp, transform)` | 静态工厂：创建绑定。参数校验失败返回 `nullptr` 并输出 `qWarning` |
...
| 方法 | 说明 |
|------|------|
| `ObservableGraphicsObject* source() const` | 获取源对象（销毁后为 `nullptr`） |
| `ObservableGraphicsObject* target() const` | 获取目标对象（销毁后为 `nullptr`） |
...
响应式绑定支持五种属性，每种属性对应 `QGraphicsObject` 的 getter/setter：

| 属性键 (`Property::`) | 含义 | 读取 (getter) | 写入 (setter) | 值类型 |
|----------------------|------|--------------|--------------|--------|
| `Pos` | 位置 | `QGraphicsObject::pos()` | `QGraphicsObject::setPos(QPointF)` | `QPointF` |
| `Scale` | 缩放 | `QGraphicsObject::scale()` | `QGraphicsObject::setScale(qreal)` | `qreal` |
| `Rotation` | 旋转 | `QGraphicsObject::rotation()` | `QGraphicsObject::setRotation(qreal)` | `qreal` |
| `Opacity` | 透明度 | `QGraphicsObject::opacity()` | `QGraphicsObject::setOpacity(qreal)` | `qreal` |
| `Visible` | 可见性 | `QGraphicsObject::isVisible()` | `QGraphicsObject::setVisible(bool)` | `bool` |

属性键定义在 `BroadItem::Property` 结构体中（`include/broaditem/reactive/ReactiveProperty.h`）：
