...
```bash
ctest --test-dir build                    # all 9 test suites
...
- **Four-stage pipeline**: Materialize (template layer → instance-layer `Node` tree) → Measure (bottom-up) → Layout (top-down) → Render.
- **Control elements are transparent**: `<for>` and `<if-has>` produce no `Node` and do **not** participate in measure/layout/render. `ColumnLayout`/`RowLayout`/`GridLayout` call `materializeChildren()` at materialization time, which structurally expands control elements into 0..N instance nodes of ordinary elements.
- **Template/instance separation**: template-layer `Element` trees are immutable after `parse()` and safely shared (`LayoutRegistry` returns `ElementPtr` as-is); per-instance state (layout rects, materialized children) lives in the instance-layer `Node` tree.
...
- **Reactive module** (`include/broaditem/reactive/`, `src/reactive/`): Item-level property synchronization between `QGraphicsItem` instances via pure meta-object driven reactive bindings. `BroadItem` inherits directly from `QGraphicsObject` and enables `ItemSendsGeometryChanges|ItemSendsScenePositionChanges` flags in its constructor so Qt's internal `Q_PROPERTY` NOTIFY signals (e.g., `xChanged()`, `yChanged()`, `scaleChanged()`, `rotationChanged()`, `opacityChanged()`, `visibleChanged()`) are emitted. `ReactiveBinding` (static `create()` factory) accepts `QObject*` source/target, auto-discovers NOTIFY signals via `QMetaProperty::notifySignal()`, reads/writes via `QObject::property()`/`setProperty()`, and handles `pos` specially via `xChanged()`/`yChanged()` connections. Supports optional transform, cycle detection, and automatic cleanup on source/target destruction. `Property` struct provides property key constants. Comments: Doxygen in headers, Chinese in source files. Not integrated with XML layout or `PropertyContext` in this version.
