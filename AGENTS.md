...
- **Reactive module** (`include/broaditem/reactive/`, `src/reactive/`): Item-level property synchronization between `QGraphicsItem` instances via signal-driven reactive bindings. `ObservableGraphicsObject` (base class for `BroadItem`) overrides `itemChange()` to emit signals when Qt properties change. `ReactiveBinding` (static `create()` factory) creates one-way bindings with optional transform, cycle detection, and automatic cleanup on source/target destruction. `Property` struct provides property key constants (`Property::Pos`, `Property::Scale`, etc.) mapping to `QGraphicsObject` getters/setters. Comments: Doxygen in headers, Chinese in source files. Not integrated with XML layout or `PropertyContext` in this version.
