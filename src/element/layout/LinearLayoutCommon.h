...
/// 仅水平轴（RowLayout）合法：XML 解析期对垂直轴报 BI-P-024 拒绝，
/// 管线内仍保留退化路径防御直接构造的 C++ 调用。
...
/// 非法 main-align/cross-align 取值报 BI-P-024 并保留原默认值。
/// @param xml 要解析的 DOM 元素。
/// @param hasLiteral 字面量属性存在性探针。
/// @param literal 字面量属性取值器。
/// @param axis 主轴方向（决定 cross-align 是否接受 "baseline"，仅水平轴接受）。
