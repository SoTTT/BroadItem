...
#ifndef BI_TL_OPTIONAL_HPP
#define BI_TL_OPTIONAL_HPP

#define BI_TL_OPTIONAL_VERSION_MAJOR 1
#define BI_TL_OPTIONAL_VERSION_MINOR 1
#define BI_TL_OPTIONAL_VERSION_PATCH 0
...
#define BI_TL_OPTIONAL_MSVC2015
#endif

#if (defined(__GNUC__) && __GNUC__ == 4 && __GNUC_MINOR__ <= 9 &&              \
     !defined(__clang__))
#define BI_TL_OPTIONAL_GCC49
#endif

#if (defined(__GNUC__) && __GNUC__ == 5 && __GNUC_MINOR__ <= 4 &&              \
     !defined(__clang__))
#define BI_TL_OPTIONAL_GCC54
#endif

#if (defined(__GNUC__) && __GNUC__ == 5 && __GNUC_MINOR__ <= 5 &&              \
     !defined(__clang__))
#define BI_TL_OPTIONAL_GCC55
#endif

#if (defined(__GNUC__) && __GNUC__ == 4 && __GNUC_MINOR__ <= 9 &&              \
     !defined(__clang__))
// GCC < 5 doesn't support overloading on const&& for member functions
#define BI_TL_OPTIONAL_NO_CONSTRR

// GCC < 5 doesn't support some standard C++11 type traits
#define BI_TL_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)                                     \
  std::has_trivial_copy_constructor<T>::value
#define BI_TL_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T) std::has_trivial_copy_assign<T>::value

// This one will be different for GCC 5.7 if it's ever supported
#define BI_TL_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T) std::is_trivially_destructible<T>::value

// GCC 5 < v < 8 has a bug in is_trivially_copy_constructible which breaks std::vector
// for non-copyable types
#elif (defined(__GNUC__) && __GNUC__ < 8 &&                                                \
     !defined(__clang__))
#ifndef BI_TL_GCC_LESS_8_TRIVIALLY_COPY_CONSTRUCTIBLE_MUTEX
#define BI_TL_GCC_LESS_8_TRIVIALLY_COPY_CONSTRUCTIBLE_MUTEX
namespace BroadItem { namespace detail { namespace tl {
  namespace detail {
      template<class T>
...
#define BI_TL_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)                                     \
    BroadItem::detail::tl::detail::is_trivially_copy_constructible<T>::value
#define BI_TL_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T)                                        \
...
#define BI_TL_OPTIONAL_CXX14
#endif

// constexpr implies const in C++11, not C++14
#if (__cplusplus == 201103L || defined(BI_TL_OPTIONAL_MSVC2015) ||                \
     defined(BI_TL_OPTIONAL_GCC49))
#define BI_TL_OPTIONAL_11_CONSTEXPR
#else
#define BI_TL_OPTIONAL_11_CONSTEXPR constexpr
#endif

namespace BroadItem { namespace detail { namespace tl {
#ifndef BI_TL_MONOSTATE_INPLACE_MUTEX
#define BI_TL_MONOSTATE_INPLACE_MUTEX
...
namespace detail {
#ifndef BI_TL_TRAITS_MUTEX
#define BI_TL_TRAITS_MUTEX
...
#define BI_TL_TRAITS_LIBCXX_MEM_FN_WORKAROUND
#endif

// In C++11 mode, there's an issue in libc++'s std::mem_fn
// which results in a hard-error when using it in a noexcept expression
// in some cases. This is a check to workaround the common failing case.
#ifdef BI_TL_TRAITS_LIBCXX_MEM_FN_WORKAROUND
...
// Trait for checking if a type is a BroadItem::detail::tl::optional
...
// Change void to BroadItem::detail::tl::monostate
template <class U>
...
struct optional_storage_base {
  BI_TL_OPTIONAL_11_CONSTEXPR optional_storage_base() noexcept
      : m_dummy(), m_has_value(false) {}

  template <class... U>
  BI_TL_OPTIONAL_11_CONSTEXPR optional_storage_base(in_place_t, U &&... u)
...
template <class T> struct optional_operations_base : optional_storage_base<T> {
  ...
  BI_TL_OPTIONAL_11_CONSTEXPR T &get() & { return this->m_value; }
  BI_TL_OPTIONAL_11_CONSTEXPR const T &get() const & { return this->m_value; }
  BI_TL_OPTIONAL_11_CONSTEXPR T &&get() && { return std::move(this->m_value); }
#ifndef BI_TL_OPTIONAL_NO_CONSTRR
...
template <class T, bool = BI_TL_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)>
...
template <class T, bool = BI_TL_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T) &&
                          BI_TL_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) &&
                          BI_TL_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T)>
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
#if defined(BI_TL_OPTIONAL_CXX14) && !defined(BI_TL_OPTIONAL_GCC49) &&               \
    !defined(BI_TL_OPTIONAL_GCC54) && !defined(BI_TL_OPTIONAL_GCC55)
  /// Carries out some operation which returns an optional on the stored
  /// object if there is one.
  template <class F> BI_TL_OPTIONAL_11_CONSTEXPR auto and_then(F &&f) & {
  ...
  /// object if there is one.
  template <class F>
  BI_TL_OPTIONAL_11_CONSTEXPR detail::invoke_result_t<F, T &> and_then(F &&f) & {
    using result = detail::invoke_result_t<F, T &>;
    static_assert(detail::is_optional<result>::value,
                  "F must return an optional");
  ...
  template <class F> BI_TL_OPTIONAL_11_CONSTEXPR auto map(F &&f) & {
  ...
  /// Carries out some operation on the stored object if there is one.
  template <class F>
  BI_TL_OPTIONAL_11_CONSTEXPR decltype(optional_map_impl(std::declval<optional &>(),
                                             std::declval<F &&>()))
  map(F &&f) & {
    return optional_map_impl(*this, std::forward<F>(f));
  }
  ...
  map(F &&f) const && {
    ...
    !defined(BI_TL_OPTIONAL_GCC54) && !defined(BI_TL_OPTIONAL_GCC55)
  /// Carries out some operation on the stored object if there is one.
  template <class F> BI_TL_OPTIONAL_11_CONSTEXPR auto transform(F&& f) & {
    return optional_map_impl(*this, std::forward<F>(f));
  }
  ...
  optional<T> BI_TL_OPTIONAL_11_CONSTEXPR or_else(F &&f) & {
    if (has_value())
      return *this;
  ...
  template <class F, detail::disable_if_ret_void<F> * = nullptr>
  optional<T> BI_TL_OPTIONAL_11_CONSTEXPR or_else(F &&f) const & {
    return has_value() ? *this : std::forward<F>(f)();
  }
  ...
  BI_TL_OPTIONAL_11_CONSTEXPR optional disjunction(const optional &rhs) & {
  ...
  BI_TL_OPTIONAL_11_CONSTEXPR optional disjunction(optional &&rhs) & {
  ...
  BI_TL_OPTIONAL_11_CONSTEXPR optional(const optional &rhs) = default;

  /// Move constructor
  ///
  /// If `rhs` contains a value, the stored value is direct-initialized with
  /// it. Otherwise, the constructed optional is empty.
  BI_TL_OPTIONAL_11_CONSTEXPR optional(optional &&rhs) = default;
 ...
 template <class... Args>
  ...
  BI_TL_OPTIONAL_11_CONSTEXPR explicit optional(
  ...
  BI_TL_OPTIONAL_11_CONSTEXPR T *operator->() {
    return std::addressof(this->m_value);
  }

  /// Returns the stored value
  BI_TL_OPTIONAL_11_CONSTEXPR T &operator*() & { return this->m_value; }
  ...
  BI_TL_OPTIONAL_11_CONSTEXPR T &value() & {
    if (has_value())
      return this->m_value;
    throw bad_optional_access();
  }
  BI_TL_OPTIONAL_11_CONSTEXPR const T &value() const & {
    if (has_value())
      return this->m_value;
    throw bad_optional_access();
  }
  ...
  template <class U> BI_TL_OPTIONAL_11_CONSTEXPR T value_or(U &&u) && {
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
  ...
  /// Carries out some operation on the stored object if there is one.
  template <class F>
  BI_TL_OPTIONAL_11_CONSTEXPR decltype(detail::optional_map_impl(std::declval<optional &>(),
                                                     std::declval<F &&>()))
  map(F &&f) & {
    return detail::optional_map_impl(*this, std::forward<F>(f));
  }
  ...
  /// If `rhs` contains a value, the stored value is direct-initialized with
  /// it. Otherwise, the constructed optional is empty.
  BI_TL_OPTIONAL_11_CONSTEXPR optional(const optional &rhs) noexcept = default;
  ...
  BI_TL_OPTIONAL_11_CONSTEXPR T *operator->() noexcept { return m_value; }
  ...
  /// \group value_or
  template <class U> BI_TL_OPTIONAL_11_CONSTEXPR T value_or(U &&u) && noexcept {
    static_assert(std::is_move_constructible<T>::value &&
                      std::is_convertible<U &&, T>::value,
                  "T must be move constructible and convertible from U");
    return has_value() ? **this : static_cast<T>(std::forward<U>(u));
...
} } } // namespace BroadItem::detail::tl

namespace std {
// TODO SFINAE
template <class T> struct hash<BroadItem::detail::tl::optional<T>> {
  ::std::size_t operator()(const BroadItem::detail::tl::optional<T> &o) const {
    if (!o.has_value())
      return 0;

    return std::hash<BroadItem::detail::tl::detail::remove_const_t<T>>()(*o);
  }
};
} // namespace std
